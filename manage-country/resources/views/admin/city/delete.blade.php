...
@section('content')
    <div class="w-full h-[88%] bg-gray-200 flex flex-col items-center justify-center">
        <h2 class="text-3xl font-bold">Are you sure of delete this city?</h2>
        <p class="mt-10 text-3xl font-bold">{{$city->name}}</p>
        <form action="{{route('city.destroy',compact('city'))}}" method="post">
            @csrf
            @method('delete')
