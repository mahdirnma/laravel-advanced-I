    ...
    <div class="w-full h-[88%] bg-gray-200 flex items-center justify-center">
        <div class="w-[90%] h-5/6 bg-white rounded-xl pt-3 flex flex-col items-center">
            <div class="w-[90%] h-1/5 flex justify-end items-center border-b">
                <h2 class="text-xl">ویرایش دوره</h2>
            </div>
            <div class="flex w-full h-4/5">
                <form action="{{route('course.edit',compact('course'))}}" method="post" class="w-full h-full flex flex-row-reverse">
                    @csrf
                    @method('put')
                    <div class="w-1/2 h-full flex flex-col items-end pr-20 relative">
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            <label for="title" class="font-semibold ml-5">: نام دوره</label>
                            <input type="text" name="title" id="title" value="{{$course->title}}" class="w-2/5 h-8 rounded outline-0 p-2 border border-gray-400">
                            @if($errors->has('title'))
                        ...
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            ...
                            <textarea name="description" id="description" cols="30" rows="7" class="rounded outline-0 p-2 border border-gray-400">
                                {{$course->description}}
                            </textarea>
                    ...
                    <div class="w-1/2 h-full flex flex-col items-end pr-20">
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            ...
                            <input type="date" name="start_date" id="start_date" value="{{$course->start_date}}" class="w-2/5 h-8 rounded outline-0 p-2 border border-gray-400">
                        ...
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            ...
                            <input type="date" name="end_date" id="end_date" value="{{$course->end_date}}" class="w-2/5 h-8 rounded outline-0 p-2 border border-gray-400">
                        ...
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            ...
                            <select name="professor_id" id="professor_id" class="w-2/5 h-8 border border-gray-400 rounded outline-0 pl-3">
                                @foreach($professors as $professor)
                                    <option value={{$professor->id}} {{$professor->id==$course->professor_id?'selected':''}}>{{$professor->firstname}} {{$professor->lastname}}</option>
                                @endforeach
                            </select>
